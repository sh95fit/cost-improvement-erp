...
>
> 마지막 갱신: 2026-06-23 (D17-10 완료 — 단위 환산 다이얼로그 3중 가드 + 미매핑 행 버튼 가드)
...
- **현재 기준 완료 지점**: Sprint 3 Phase 1.7 / D17-10 (단위 환산 3중 가드 + UnitMaster.code 기반 매칭 일관화)
- **최근 완료**:
...
- **다음 진행 항목**: **Fix-R2 잔여** (Step 5 품목별 예상 입고일 리스트업·Step 4 라인업 뷰 백엔드 lineupBreakdown 추가) → Phase 4-C (상세보기 + 상태 전이 다이얼로그) → Phase 4-D (수동 발주 다이얼로그)
- **현재 블로커**: 없음
