...
| P20 | **MealPlan 역방향 전이 가드** — COMPLETED→IN_PROGRESS·IN_PROGRESS→CONFIRMED 역방향 시 연계된 Consumption 이 CONFIRMED 이면 차단 (`ConfirmedConsumptionExistsError`). PENDING 이하만 있으면 PENDING Consumption 삭제 + FINAL/ESTIMATED MR soft-delete + 관련 InventoryReservation release 후 상태 원복. 재확정 시점에 R5-P/R5-R1 재실행 | 2026-07-23 신설 (사용자 확인). R12 재편집 유예 정책의 순환 흐름 정합. 구현: `revert-guard.service.ts` (R5-R1-B) |
...
| S4-3-c-R5  | ⚠️ 부분 폐기 | R5-P만 유지, ~~R5-R1 폐기 (2026-07-30)~~ | R5-P (`autoCreatePendingConsumptionHeaders`) 유지. ~~R5-R1 (`autoReserveFromMaterialRequirements`)~~ §0 원칙 1 위반으로 **폐기**. 예약 생성은 R14 입고 확정 시점으로 완전 이관. Phase 1 커밋에서 서비스 파일 삭제 + `updateMealPlanGroup` 호출부 제거 + 테스트 3종 삭제 예정. 상세: 감사서 §0, §9-1, §9-14 |
| S4-3-c-R5-R1-B | ⬜ | MealPlan 역방향 전이 가드 + 연쇄 정리 | `revert-guard.service.ts` 신설 (P20). CONFIRMED Consumption 존재 시 throw, 없으면 PENDING Consumption 삭제 + MR soft-delete + Reservation release |
