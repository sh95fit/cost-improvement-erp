...
## 풀려는 진짜 문제
식단 기반으로 발주·입고·사용을 정확히 추적하여,
일자별 라인업 원가를 신뢰성 있게 산출하고,
...
## 헌법 (P1~P10)
1. BOM·식단 = 회사 계층 전용. 그 외 모든 운영은 회사/공장/제조라인 롤업.
2. 회사는 재고를 보유하지 않는다.
3. 사용 처리는 FACTORY에서만.
4. 입고 확정 → 즉시 사용 예약 → 출고일 처분 결정(USED/RETURNED/DISPOSED) → 일반 재고.
5. WAREHOUSE = 사전 발주·장기 보관, FACTORY = 직접 사용.
6. 재고 실사는 StockGrade(A/B/C) 주기.
7. 원가 = 사용량 기반(실시간) × 스냅샷 기반(재무) 하이브리드 크로스체크.
8. 모든 트랜잭션 1급 키 = outboundDate.
9. 발주 = 식단 기반(정상) / 수동(예외, isManual=true) 분리.
10. 단가 변경 = 입고 확정 시점에 PriceHistory 자동 적층.
...
## 🧭 SOLUTION CORE (불변 원칙) — 2026-06-15 정립

이 박스에 적힌 원칙들은 모든 후속 설계의 상위 제약. 어떤 단계든 위배되는
구현은 즉시 폐기/재설계한다.

| # | 원칙 | 비고 |
|---|---|---|
| P1 | **발주는 식단(MealPlan) 기반**. 공급사 선두 선택 흐름 금지 | 수동 발주 포함 |
| P2 | Roll-up: Company → Location(공장/창고) → ProductionLine | 모든 운영 도메인 |
| P3 | **재고는 공장/창고에만 존재**. Company는 무재고 | 회계 단위는 Company |
| P4 | 사용(Consumption)은 **공장에서만** 발생 | ProductionLine 단위 추적 |
| P5 | 입고 확정 시 예약(Reservation) 생성 → 사용/폐기/재고전환으로 소진 | |
| P6 | BOM/MealPlan은 Company 레벨에서만 관리 | 공장은 받아서 실행만 |
| P7 | 비용 계산 = (일별 사용 기반 운영원가) + (월말 스냅샷 회계원가) 하이브리드 | |
| P8 | 발주 PK 추적 키는 outboundDate(실제 출고일), 회계 키는 snapshotDate | |
| P9 | 단가 변경 이력은 **입고 시점**에 SupplierItemPriceHistory 적층 (자동) | |
| P9' | 단, 발주서 작성/수정 단계의 unitPrice 편집은 `PurchaseOrderItem.unitPrice`에만 영향 — `SupplierItem.currentPrice` 불변 | "기준가 갱신" 명시 액션일 때만 마스터 갱신 + 적층 |
| P10 | 모든 레벨이 발주→입고→사용→재고→폐기 전체 라이프사이클 지원 | |

### Sprint 3 일정 재정렬 (2026-06-15)
...
| **0-B** | SupplierItem action 시그니처 정리 + 문서 갱신 | 🔄 진행 중 | - |
| **0-C** | DB 점검 (PurchaseOrder 행 수) + PO location/productionLine 마이그레이션 SQL 확정 | ⬜ | - |
| **1.5** | PurchaseOrder 스키마 확장 (locationId, productionLineId) | ⬜ | - |
| **4-B′** | 식단 기반 발주 위저드 (MealPlanGroup → 공장/라인 → 자재 → 공급사 상품 매핑 → 공급사별 PO 자동 분할) | ⬜ | - |
| 4-C | 상세보기 + 상태 전이 다이얼로그 | ⬜ | - |
| 4-D | 수동 발주 다이얼로그 (4-B′ 컨벤션 재사용, isManual=true) | ⬜ | - |
| 5 | ReceivingNote 백엔드 (입고 시 자동 PriceHistory 적층 — P9) | ⬜ | - |
| 6 | ReceivingNote UI | ⬜ | - |
