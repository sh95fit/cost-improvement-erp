...
> 마지막 갱신: 2026-06-11 (Phase 9-D-Sym 완료 — MealPlanSlot 수량 대칭 분리 + UI/검증/산출 정합. Sprint 2 종결 진입)
...
### 현재 상태 요약
- **현재 기준 완료 지점**: Sprint 2 / Phase 9-D-Sym 완료 — MealPlanSlot 수량 대칭 분리 + UI/검증/산출 정합
- **현재 프로젝트 상태**: Sprint 2 종결 진입 — 아카이브 후 Sprint 3 (발주 산정) 착수
- **현재 블로커**: 없음
...
- **Phase 9-A-2/3** 완료: MaterialRequirement service/action (generateMaterialRequirements 구현)
- **Phase 9-B** 완료: material-requirement 페이지 UI
- **Phase 9-C-Fix-H** 완료 (commit `822c288d`): slot.quantity 우선 산출 + 미스매치 경고 누적
- **Phase 9-C-Fix-B/B2** 완료 (commits `d56b18a3`, `a28431b0`): 레시피 코드 생성 안정화, Prisma soft-delete 우회
- **Phase 9-C-Fix-K1** 완료 (commit `1fb666e1`): 슬롯 수량 검증 차단 (MealPlan 단위 — 결함 발견)
- **Phase 9-C-Fix-K2** 완료 (commit `79e545cf`): DRAFT→상태 전환 시 검증 차단 (MealPlan 단위 — 결함 발견)
- **Phase 9-C-Fix-H2 + K3** 완료 (commit `91c5696e`): UI prefill + 실시간 검증 배지 + handleStatusChange 가드 (결함 동일)
- **Phase 9-C-Fix-K3-Cleanup** 완료 (commit `6200d158`): 배지 중복 렌더링 제거
- **Phase 9-C-Fix-R1** ✅ 완료: 레시피 그룹 단위로 검증/산출 재설계
  - ✅ R1-1/2: validateSlotQuantitiesForMealPlan 시그니처 + 호출부 정리
  - ✅ R1-3: 자재 산출 본 루프를 레시피 그룹 단위로 재작성 (commit `ee3dfdfe`)
  - ✅ R1-4: action 레이어 에러 메시지 매핑 + formatSlotQuantityError (commit `d09b2e85`)
  - ✅ R1-5: UI(page.tsx) `checkMealPlanSlotQty` BY_RECIPE 그룹 결과, 배지 분리(OK_FALLBACK/OK_DISTRIBUTED/PARTIAL_INPUT/SUM_MISMATCH/MULTI_LINE_REQUIRES_QUANTITY), `ALLOWED_FORWARD_TRANSITIONS` 가드, SelectContent 동적화 (commit `6554362c`)
  - ✅ R1-6: state-guard 정비 + DRAFT 자동 승격 + MaterialRequirement 산출 상태 가드 (commit `6762c97d`)
  - 남은 보강: R1-5b(헤더 4종 버튼 + OK_FALLBACK 배지 숨김)는 Phase 9-D-Sym Commit ③ UI에 흡수
- **Phase 9-D-Sym** ✅ 완료: MealPlanSlot 수량 대칭 분리 (estimatedQuantity/finalQuantity) + UI/검증/산출 정합 + STATUS_LABEL CONFIRMED→"준비중"
  - 배경: 예상식수와 동일하게, 확정식수도 동일 recipeId 슬롯 2개 이상이면 수량 입력 의무화 필요
  - 정책: `MealPlanSlot.quantity` → `estimatedQuantity` 개명 + `finalQuantity Int?` 추가
  - 검증 대칭: CONFIRMED→IN_PROGRESS는 estimated, IN_PROGRESS→COMPLETED는 final 사용
  - 자재 산출: `countSource=ESTIMATED`는 estimatedQuantity, `countSource=FINAL`은 finalQuantity 사용
...
- Phase 9-A — 소요량 자동 산출 서비스 (materialRequirement) ✅ 완료
  - ✅ 9-A-0/1/1.5 스키마·마이그레이션·Zod (이전 라운드)
  - ✅ 9-A-2 service (generateMaterialRequirements + UPSERT/UNDELETE/soft-delete diff)
  - ✅ 9-A-3 action
  - ⏳ 9-A-4 테스트 (후속)
- Phase 9-B — material-requirement UI 페이지 ✅ 완료
- Phase 9-C — 슬롯 수량 정합성 정책 (Fix 시리즈) ✅ 완료
  - ✅ 9-C-Fix-H (commit `822c288d`): slot.quantity 우선 산출
  - ✅ 9-C-Fix-K1 (commit `1fb666e1`): 산출 단계 검증 차단 (단, MealPlan 단위 — 결함)
  - ✅ 9-C-Fix-K2 (commit `79e545cf`): 상태 전환 단계 검증 차단 (동일 결함)
  - ✅ 9-C-Fix-H2+K3 (commit `91c5696e`): UI prefill + 실시간 배지 (동일 결함)
  - ✅ 9-C-Fix-K3-Cleanup (commit `6200d158`): 배지 중복 제거
  - ✅ 9-C-Fix-R1-3 (commit `ee3dfdfe`): 자재 산출 본 루프 레시피 그룹 단위 재작성 + 책임 단일화
  - ✅ 9-C-Fix-R1-4 (commit `d09b2e85`): action 레이어 에러 메시지 매핑
  - ✅ 9-C-Fix-R1-5 (commit `6554362c`): UI 배지/가드/SelectContent 동적화
  - ✅ 9-C-Fix-R1-6 (commit `6762c97d`): state-guard + DRAFT 자동 승격 + MR 산출 가드
- Phase 9-D-Sym — 확정식수 슬롯 컬럼 분리 + 검증 대칭화 ✅ 완료
  - ✅ 9-D-Sym-Schema: `MealPlanSlot.quantity` → `estimatedQuantity` 개명 + `finalQuantity Int?` 추가 + 마이그레이션 백필
  - ✅ 9-D-Sym-Service: meal-plan.service / material-requirement.service / Zod schema 대칭 검증 추가
  - ✅ 9-D-Sym-UI: page.tsx 예상수량/확정수량 분리 입력 + 두 줄 셀 표시 + checkMealPlanSlotQty countSource 분기
  - ✅ 9-D-Sym-Status: R1-5b 헤더 4종 상태 전환 버튼 + 사전 검증 (handleStatusChange에 흡수)
  - ✅ 9-D-Sym-Label: STATUS_LABEL CONFIRMED → "준비중" (enum 값 유지, 라벨만 조정)
  - ⏳ 후속 메모: `collectGroupSlotQtyIssues` 가 IN_PROGRESS→COMPLETED 가드용 countSource="FINAL" 분기를 갖도록 확장 필요 (현재는 ESTIMATED만 검증). 확정식수에 분배 정책을 도입하는 시점에 작업.
- Phase 9-D-Acc — 부자재(MealPlanAccessory) 사용처 지정 및 산출 ⬜ 신규 트랙
  - 9-D-Acc-1: schema.prisma에 `MealPlanAccessory.productionLineId` 컬럼 추가 + 마이그레이션
  - 9-D-Acc-2: 부자재 추가/편집 UI에 ProductionLine 선택 추가
  - 9-D-Acc-3: 부자재 산출 로직 (PER_MEAL_COUNT는 estimated→final 흐름, FIXED_QUANTITY는 고정값)
  - 9-D-Acc-4: 잔존 필드 `quantity Float` 정리 결정
  - 9-D-Acc-5: 식단 밖 독립 발주 트랙 (공장이 소모품 직접 발주)
- Phase 9-E — PO 자동 생성 연결 (대기, 기존 Phase 9-C의 PO 항목 분리)
- Phase 10 — 테스트 작성
- Phase 10‑Lint (예정, 별도 스프린트)
  - 10‑1: react-hooks/set-state-in-effect 24건 일괄 수정 (React 19 패턴 적용)
  - 10‑2: 테스트 파일 no-explicit-any 72건 — mock 타입 헬퍼 도입
  - 10‑3: react/no-unescaped-entities 2건 (meal-plans/page.tsx)
  - 10‑4: no-unused-vars warnings 11건 정리
