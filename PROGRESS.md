...
>
> 마지막 갱신: 2026-07-09 (Sprint 3.5 Phase S3.5-2 완료. 다음: Phase S3.5-3 테스트·문서)
...
- **최근 완료 항목**:
  - Sprint 3.5 Phase S3.5-0 (스키마 보강, 커밋 `95112583`)
  - Phase S3.5-1 (서비스·스키마·액션 라인업 검증 확장 + 레거시 픽스처 정리, 커밋 `95112583`·`8ac522b`)
  - **Phase S3.5-2 (수동 발주 UI, 커밋 `{S3.5-2a SHA}`·`{S3.5-2b SHA}`·`{S3.5-2c SHA}`·`{S3.5-2d/e SHA}`·`{layout/redirect SHA}`)** ✅
- **다음 착수 항목**: **Sprint 3.5 Phase S3.5-3 (테스트·문서)** → Sprint 4 Phase S4-0
...
> `lineupId` 는 Phase S3.5-0 에서 신규 추가됨 (2026-07-08 마이그레이션 `add_purchase_order_lineup`).
...
- 액션(`purchase-order.action.ts`): `createPurchaseOrderAction`·`updatePurchaseOrderAction` 의 에러 매핑에 4종 라인업 오류 한글 문구 추가.
- 테스트: 레거시 픽스처 6건 중 자동 번호·총액 계산 관련 3건에서 `isManual: true` 제거 (기본값 `false` 적용). 나머지 3건(location/line 실패 검증) 은 Phase S3.5-3 에서 라인업 신규 테스트와 함께 정리 예정.
...
### ✅ Phase S3.5-2 (수동 발주 UI) — 2026-07-09 완료
...
- **마스터 미변경 (P9')**: 수동 발주 아이템은 `setAsDefault: false` 로 SupplierItem 마스터의 currentPrice 를 갱신하지 않음.
...
- **`limit` 상한 준수**: `getMaterialsAction` limit ≤ 100, `getProductionLinesAction` limit ≤ 200, `getLineupsAction` limit ≤ 200 (각 스키마의 `.max()`).
...
- `src/tests/purchase-order-batch.service.test.ts` — 기존 케이스에 `isManual: false` 명시 (S3.5-2b 후속)

**미완 이슈 (S3.5-3 에서 처리)**
- 수동 발주 전용 배치 서비스 테스트 케이스 (현재 배치 테스트는 모두 `isManual: false` 만 검증)
- `manual-purchase-order-form.tsx` 컴포넌트 테스트 (선택)
- 도메인 문서 (`docs/domain/PURCHASE_ORDER.md` 또는 유사 파일)에 수동 발주 경로 명문화

### ⏭ Phase S3.5-3 (테스트·문서) — 착수 예정

**목표**: Sprint 3.5 마감. 수동 발주 도메인에 대한 회귀 방지 커버리지 + 도메인 문서 정본화.

**작업 항목**
1. **배치 서비스 수동 발주 테스트** (`src/tests/purchase-order-batch.service.test.ts` 에 `describe("Manual mode (S3.5-3)")` 블록 신설)
   - `isManual: true` + `lineupId` 누락 → `LINEUP_REQUIRED_FOR_MANUAL`
   - `isManual: true` + 존재하지 않는 `lineupId` → `LINEUP_NOT_FOUND`
   - `isManual: true` + 타 회사 `lineupId` → `LINEUP_COMPANY_MISMATCH`
   - `isManual: true` + 비활성 `lineupId` → `LINEUP_INACTIVE`
   - `isManual: true` + `mode: "DELTA"` → `MANUAL_PO_ONLY_NEW_MODE`
   - `isManual: true` + `mode: "REPLACE"` → `MANUAL_PO_ONLY_NEW_MODE`
   - `isManual: true` + 다중 공급업체 items → 공급업체 수만큼 PO 분리, 모두 `isManual: true`, `sourceType: "MANUAL"`, `lineupId` 세팅
   - `isManual: true` + `setAsDefault: false` 강제 확인 → SupplierItem.currentPrice 미갱신
   - `isManual: true` + 그룹 키에 `lineupId` 포함 검증 → 같은 supplier/loc/line 이라도 다른 lineup 이면 별도 PO
2. **액션 레이어 스모크 테스트** (선택, mock 최소화하여 액션-서비스 통합 확인)
3. **PROGRESS.md 헌법 P1' 항목**에 수동 발주 구현 상태 링크 추가
4. **`docs/domain/PURCHASE_ORDER.md`** (신규 또는 갱신) — 발주 두 경로 (식단 기반/수동) 흐름도, 스키마 필드 의미, 배치 서비스 그룹 키 규칙, `isManual` 이 sourceType/setAsDefault/mealPlanGroupId 에 미치는 영향 명시
5. **Sprint 3.5 종결 처리** — 통계 확정, `docs/progress/SPRINT3.5.md` 로 이관, `SCHEMA_COVERAGE.md` 변경 이력 라인 추가, PROGRESS.md 요약본 1줄로 축약, 커밋 메시지 `docs(progress): close sprint 3.5 — archive to SPRINT3.5.md`
...
|---|---|---|
| 수동(독립) 발주 | Sprint 3.5 | 🟡 스키마·서비스·액션 완료(S3.5-0·S3.5-1), UI(S3.5-2) 착수 대기 |
