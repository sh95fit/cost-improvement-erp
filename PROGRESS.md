...
> 마지막 갱신: 2026-06-11 (Sprint 2 종결 — 이력 docs/progress/SPRINT2.md 로 아카이브, Sprint 3 착수 대기)
...
### 현재 상태 요약
- **현재 기준 완료 지점**: Sprint 2.5 (단위 관리 중앙화) + Sprint 3 사전 결정사항 + 스키마 보강
- **현재 프로젝트 상태**: Sprint 3 Phase 1 (PO Zod 스키마 작성) 착수 대기
- **현재 블로커**: 없음
- **최근 보강 (이번 커밋)**:
  - ConsumptionItem: disposition / disposalReason / disposalNote + 부자재 표현(itemType, subsidiaryMasterId) 필드 추가
  - InventoryLot: materialMasterId nullable 전환 (부자재 재고 표현)
  - PurchaseOrder: submittedAt / approvedByUserId / cancelledAt / cancelledByUserId / cancelReason 필드 추가
  - User: PO 관계 분리 (createdBy / approvedBy / cancelledBy)
  - enum ConsumptionDisposition / DisposalReason 신설
  - 마이그레이션은 Sprint 3 Phase 1 진입 시점에 실행
