...
| S4-3-c-R6-B-2 | ⬜ | Pre-flight `getAvailableStock` 호출 리팩토링 | 감사서 §10-9, §10-13 근거. `MergedItem`/`LayerBItem` 타입 확장, `mergeItems` 병합 키 확장 (lineupId/productionLineId 추가), 자재는 `getAvailableStock` · 부자재는 `getSubsidiaryAvailableForConsumption` (§10-12 신설) 분기 |
