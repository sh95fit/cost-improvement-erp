...
>
> 마지막 갱신: 2026-07-16 (Sprint 4 Phase S4-3-c-3 완료 — 사용 이력 목록 UI + 신규 진입 다이얼로그. 다음: S4-3-c-4 사용화면 재설계 — 라인업별 행 분리, 발주단위 노출, 폐기 분리 저장, FIXED_QUANTITY 부자재 처리, 미등록 자재 보정)
...
- **c-4-3** — 사용 화면 테이블 재설계 (+ 미등록 자재 보정 다이얼로그 포함)
  - `consumption-draft-form.tsx` 전면 개편: 편집 가능 셀 (`finalUsedQty`, `disposalQty`), 폐기 사유 인라인 셀렉트, 라인업별 그룹 헤더 (엑셀식 병합)
  - 실시간 합산 검증: 같은 `itemId` 그룹 내 `Σ(finalUsedQty + disposalQty) ≤ totalAvailable` — 초과 시 붉은 테두리 + 상단 배너
  - 미등록 자재 (환산 계수 없음) 행: 우측 `⚙ 발주단위 설정` 버튼 → 인라인 미니 다이얼로그 (`unit-conversion-mini-dialog.tsx` 신설)
    - 필드: 공급업체, 발주단위, 환산 계수
    - 저장 시 기존 `createUnitConversionAction` 호출 → 해당 행 즉시 재계산
    - 발주서 생성 단계 로직 재사용, 별도 서비스 신설 없음
- **c-4-4** — `confirmConsumption` 스키마·서비스 확장
  - 입력 스키마: `{ itemId, itemType, lineupId?, finalUsedQty, disposalQty, disposalReason?, disposalNote?, orderUnit, orderUnitFactor }`
  - 저장 로직: `finalUsedQty > 0` → USED 행 1건 (FIFO), `disposalQty > 0` → DISPOSED 행 1건 (FIFO). 각 행 독립 실행.
  - 검증: `finalUsedQty + disposalQty ≤ totalAvailable` (Pre-flight), `disposalQty > 0 → disposalReason 필수`, `disposalReason=OTHER → disposalNote 필수`
  - 저장 단위: 입력값(`orderUnit`) → `baseUnit` 환산 후 `ConsumptionItem.consumedQty` 저장
- **c-4-5** — 에러 UX 통일
  - `/consumption/new` 페이지의 페이지 전체 오류 박스 → `/consumption?error=...` 로 redirect + 리스트 페이지 toast 표시
