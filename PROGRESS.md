...
>
> 마지막 갱신: 2026-07-30 (Sprint 4 Phase S4-3-c R0·R1·R2·R3 완료 — R0 감사서 편입, R1 Shipping 잔재 정리, R2 발주단위 개념 폐기·UI 다이얼로그 3종 폐기·라벨 정합화, R3-a 필드명 batch rename, R3-b `ConsumptionHeader` 모델·enum 2종 신설 + `ConsumptionItem.headerId`/`supplierItemId` FK 추가, R3-c `confirmConsumption` 트랜잭션 내 Header upsert 편입 (idempotent AUTO_MEAL_PLAN, PENDING→CONFIRMED), R4 기존 데이터 purge 는 R3-b reset 처리로 흡수되어 N/A 종결 (검증 스크립트 `prisma/scripts/verify-r4-consumption-residual.ts` 로 4개 지표 0건 확인, 프로덕션 배포 시점 별도 재검토), 2026-07-23 R5-0 사전 준비 (감사서 §9 R5 상세 설계 확정, P16 공식 `referenceType` `MEAL_PLAN_SLOT`→`MATERIAL_REQUIREMENT` 정정 2곳, R14/R15 페이즈 분리 신설, L911-913 유령 행 정리), 2026-07-23 R5-R1 진행 (Commit 1 감사서 §9-10 매핑 + 서비스 스켈레톤 2건 `f37adee`, Commit 2 `autoCreatePendingConsumptionHeaders` 실구현 + mock 보강 (`consumptionHeader`/`consumptionItem`/`consumptionLotDetail` 3건) + 테스트 4종 `7aaf064`, Commit 3 `autoReserveFromMaterialRequirements` 실구현 + 테스트 6종 `a45c641`, Commit 4 §9-4 α3 정합 정정 — auto-reserve release Phase 제거 (revert-guard 이관), `revert-guard.service.ts` 스켈레톤 신설 (P20·R5-R1-B, 실구현 R12), 테스트 6종→5종 축소 `34bb71e`, Commit 5 R5 통합 완료 — 감사서 §9-10 Reservation 매핑 §9-4 α3 정합 정정, `updateMealPlanGroup` 시그니처에 `actorUserId` 추가, `meal-plan.action.ts` `session.userId` 전달, `isForwardToInProgress` 조건부로 `autoCreatePendingConsumptionHeaders` + `autoReserveFromMaterialRequirements(ESTIMATED)` 호출 삽입, `meal-plan.service.test.ts` mock/beforeEach/기존 5종 호출부 확장 + 신규 3종 추가, PROGRESS.md R5 행 ✅). 상세 감사 이력: `docs/audits/2026-07-21-consumption-receiving-audit.md`, 2026-07-24 R6-Pre 정정 (감사서 §10 SK 정합 정정 — D-R6-f α: `stockKeepingFree` 필드 삭제 · SK Lot 예약 대상 아님·`freeStock` 자연 합산 명시, D-R6-e α: `InventoryReservation(referenceType,referenceId)` 인덱스 R6-Pre-2 선행 마이그레이션 확정, §10-2 lineupId=null 매칭 사문화 조항 정리), 2026-07-24 R6-Pre-2 완료 (InventoryReservation `[referenceType, referenceId]` 복합 인덱스 마이그레이션 `20260724015728_s4_3_c_r6_pre_2_add_inv_reservation_reference_idx` 적용 · Supabase DB sync 완료, D-R6-e α 실행), 2026-07-24 R6-A 완료 (available-stock.service.ts 신설 219줄 + 테스트 7종 254줄 · P16 정본 aggregate-level 구현 · breakdown 4필드 {reservedSameAxis, freeStock, reservedOtherDate, reservedOtherAxis} · MR referenceType join 분류 · SK/SUBSIDIARY/purchaseKind=NULL lot freeStock 자연 합산 · 음수 clamp · 전체 vitest 590 passed 회귀 없음)), 2026-07-24 R6-Pre-3 완료 (감사서 §9-12 도메인 축 매트릭스 신설·§9-13 예약재고 소비 축 정책 신설·§10-9 R6-B 상세 스펙 개정·§10-11(4) R6-A-Ext 폐기 명시·§10-12 부자재 헬퍼 스펙 신설·§10-13 mergeItems 병합 키 확장 정책 신설·§10-14 Layer B UI 라인업 필터 스펙 신설·§10-15 ConsumptionItem.lineupId NOT NULL 즉시 도입 결정 박제·§10-16 R6-B 페이즈 세분화·§10-17 R7-a 선행 의존성·§10-18 R6-A-Ext 폐기 결정 박제·§11 R6 관련 미해결 항목 매핑 신설. PROGRESS.md L323 Q6 결정 확정 (파생 컬럼 → NOT NULL 즉시 도입), R6 행에 R6-B-1~4·R7-a·R7-b~e 세분화. Q1~Q7 사용자 확인 완료. 코드 변경 없음 (문서·감사서 박제 전용 커밋)), 2026-07-24 R6-B-1 완료 (`ConsumptionItem.lineupId` NOT NULL FK + 인덱스 2종 마이그레이션 `20260724094111_s4_3_c_r6_b_1_add_consumption_item_lineup_id` 적용, `Lineup.consumptionItems` back-relation 추가, `MergedItem`/`itemKey`/`mergeItems`/`detectLayerADrift`/`perItemLots` 병합 키에 lineup 축 확장 (감사서 §10-13), `ConsumptionItem.create` 에 `lineupId` 전달, Layer A 사전 non-null 검증 (§9-13-e), Layer B 임시 guard 삽입 (R6-B-3 대기, §10-14), `InvalidLayerBItemError.LAYER_A_LINEUP_MISSING` 코드 + `confirm-consumption.action.ts` case 매핑 추가, TypeScript 0 에러 확인, `SCHEMA_COVERAGE.md` #52 갱신), 2026-07-24 R6-B-2 완료 (`available-stock.service.ts` 에 `getSubsidiaryAvailableForConsumption` 신설 [§10-12, `{available, lots}` 반환, 예약 차감 없이 remainingQty 단순 합산], `confirm-consumption.service.ts` Pre-flight 루프를 자재 `getAvailableStock` / 부자재 `getSubsidiaryAvailableForConsumption` 분기로 리팩토링, FIFO 차감용 Lot 배열은 별도 쿼리 유지 [§10-3 자재 헬퍼 스펙 상 lots 미포함], `getAvailableQty` 는 FIFO 차감부에서 유지 [§10-5]. 전체 vitest 590 passed 회귀 없음), 2026-07-24 R6-B-4 완료 (`src/tests/confirm-consumption.service.test.ts` 신설 12 테스트 케이스 · 4 describe 블록 — Layer B guard [§10-14] 1종 · Pre-validation [P11/P14: QUANTITY_NEGATIVE, QUANTITY_OVERFLOW, DISPOSAL_REASON_REQUIRED, DISPOSAL_NOTE_REQUIRED] 4종 · Drift 감지 [D2=α: StaleDraftError] + Layer A lineupId/productionLineId non-null guard [§9-13-e: LAYER_A_LINEUP_MISSING] 3종 · mergeItems 키 확장 검증 [§10-13: 동일 (itemType,itemId) 다른 lineup → 별도 ConsumptionItem] + Helper 분기 [§10-3 getAvailableStock / §10-12 getSubsidiaryAvailableForConsumption] 4종. Mock 전략: vi.mock 6모듈 (`@/lib/prisma`, `consumption-draft.service`, `cooking-plan-upsert.service`, `available-stock.service`, `reservation.service`, `audit`) + `setupHappyPath()` 헬퍼로 트랜잭션 내부 8개 Prisma 호출 일괄 세팅. `src/tests/mocks/prisma.ts`에 `cookingPlan: createModelMock()` 한 줄 추가 (인프라 확장, 기존 590 테스트 영향 없음). vitest 604 passed / 2 skipped [기존 590 + 신규 12, 2 skipped 유지], type-check 0 errors 회귀 없음. R6-B 페이즈 4-tier [R6-B-1 schema · R6-B-2 service · R6-B-3 UI 대기 · R6-B-4 tests] 중 마지막 단계 완료. 상위 커밋: R6-B-1 `71636b2` · R6-B-2 `33470e2`), 2026-07-27 R7-a 완료 (감사서 §10-17,§11-1 근거. `src/lib/auth/scope.ts` 신설: `getUserScope`/`applyScopeFilter`/`assertScopeAccess` 3개 유틸 + `ScopeAccessDeniedError`. 현 스키마는 `UserScope.role`만 보유 (companyId+role, LOCATION/PRODUCTION_LINE 컬럼 없음) → COMPANY 레벨만 실동작하며 audit §10-17 시그니처는 선반영 (TODO). `src/tests/scope.service.test.ts` 12 케이스 신설. `src/tests/mocks/prisma.ts`에 `userScope: createModelMock()` 1줄 추가. vitest 616 passed / 2 skipped [기존 604 + 신규 12], type-check 0 errors 회귀 없음. R6-B-3 UI 페이즈 선행 의존성 해제. 상위 커밋: R6-B-4 `2f91400`), 2026-07-27 R6-B-3 완료 (감사서 §10-14, §10-17 근거. Layer B 라인업 선택 UI + 서버 Zod + 스코프 필터 통합. 신규 액션 `getScopedLineupsForConsumptionAction` (MaterialRequirement distinct + applyScopeFilter), 신규 스키마 `confirm-consumption.schema.ts` (Zod non-null 강제), `confirmConsumptionAction` Zod parse + `assertScopeAccess` 추가, `LayerBItem` 타입에 `lineupId`/`productionLineId` 필드, UI Editor 라인업 컬럼 신설, page.tsx `Promise.all` 병렬 조회, `consumption-draft-form` 클라 검증. 테스트 `scoped-lineups.action.test.ts` 5 케이스. vitest 621 passed / 2 skipped [기존 616 + 신규 5], type-check 0 errors 회귀 없음. R6-B 4-tier 완료 [R6-B-1 schema · R6-B-2 service · R6-B-3 UI · R6-B-4 tests]. 상위 커밋: R7-a `1fe1ff6`), (S4-3-c-R15 §9-5·§11-3 MealPlan CANCELLED 전이 시 활성 Reservation 일괄 release: 신규 서비스 `release-reservations-on-meal-plan-cancel.service.ts` + `updateMealPlanGroup` `isTransitionToCancelled` 트랜잭션 블록 통합 + 테스트 3 케이스. vitest 624 passed / 2 skipped, type-check 0 errors. 상위 커밋: R6-B-3 `d8f3764`), 2026-07-30 R5-R1 폐기 결정 + §0 예약 3대 원칙 박제 (감사서 커밋 `68e6726`. Sprint 4 진행 중 예약 도메인 반복 재해석 이력 종결. 원칙 1: 예약은 실물 자재 입고 확정 시점에만 발생 → R14 유일 지점. 원칙 2: 독립 누적, receivedQty 그대로. 원칙 3: 재고 부족 검증은 confirmConsumption FIFO 차감 루프 전용. R5-R1 `autoReserveFromMaterialRequirements` 원칙 1 위반으로 폐기 (Phase 1 커밋에서 코드 제거 예정). 감사서 §0/§9-1/§9-2/§9-4/§9-5/§9-7/§9-8/§9-14/§11-2/§11-3 개정), 2026-07-30 예약 도메인 리팩터 실행 완료 — Phase 0a 감사서 개정 `68e6726` → Phase 0b PROGRESS.md 동기화 `1a99de2` → Phase 1 R5-R1 코드/테스트 폐기 `68243b2` (vitest 622→617, 서비스+테스트 파일 2건 삭제, meal-plan.service.ts import/호출 정리, meal-plan.service.test.ts mock/assertion 정리) → Phase 0c 감사서 §9-14-3 스키마 관계 방향 hotfix `b000b66` (Lot↔ReceivingNoteItem FK 방향 H-89 실측 정정) → Phase 2 R14 서비스 도입 + confirmReceivingNote 트리거 연결 `417a4bf` (auto-reserve-on-receiving-confirm.service.ts 신설, receiving-note.service.ts wire, 기존 receiving-note 테스트 mock 확장) → Phase 3 R14 전용 테스트 스위트 `8768eaa` (8 케이스, vitest 617→625) → Phase 4 R15 서비스 단위 테스트 스위트 `ae71e37` (7 케이스, vitest 625→632). §0 3대 원칙 최종 정합 검증: 원칙 1 (R14 유일 지점) ✓ / 원칙 2 (독립 누적) ✓ / 원칙 3 (재고 부족 검증은 confirmConsumption 전용, 조기 검사 경로 소멸) ✓, 2026-07-30 R8 β방향 사전 박제 Cycle 1 완료 `656c4a6` (감사서 §9-1 R8-c 스코프 추가·§9-4 순방향 전이 개정 [IN_PROGRESS 진입 시 ESTIMATED 기반 PENDING Item 생성, COMPLETED 진입 시 FINAL 기반 재계산]·§9-10-i Item 자동 갱신 규칙 신설·§9-15 R8-c 상세 설계 신설·§9-16 가드 축 분리 신설 [assertMealPlanInProgressForDraftView 신규 vs assertMealPlanCompletedForConsumption 기존]·§10-14 진입 조건 [Layer B COMPLETED 이상] 추가. 사전 결정 3건: 결정 1 β 확정 [IN_PROGRESS 부터 draft 조회 허용]·결정 2 [Layer B 는 COMPLETED 이상]·결정 4 Q [FINAL 진입 시 PENDING Item 전량 덮어쓰기, CONFIRMED·MANUAL_ADDITION 불변]. UI 사용 관리 목록 공백 원인 규명: R5-P 는 Header 만 생성하고 Item 은 R8-c 소관인데 R8 미착수 상태였음. 문서 개정만 반영, 코드 변경 없음. Cycle 2 착수 대기 [가드 함수 신설]), 2026-07-30 R8 β방향 Cycle 2 완료 `120f3ca` (가드 함수 축 분리 코드 신설: `assertMealPlanInProgressForDraftView` 신규 [IN_PROGRESS/COMPLETED 허용] + `MealPlanNotReadyForDraftViewError` 신규 + `MEAL_PLAN_NOT_READY_FOR_DRAFT_VIEW` 에러 코드 상수 추가. 기존 `assertMealPlanCompletedForConsumption` 무변경. wire 는 Cycle 3-B 예정. +91 insertions / -0 deletions, vitest 632 passed / 2 skipped 회귀 없음), 2026-07-30 R8 Cycle 3-A hot-fix 완료 `27f5eab` (감사서 §9-15 R8-c 폐기 [draft 서비스 MR 기반 실시간 재계산 실측 확인, PENDING Item 사전 생성 불필요] + 연쇄 폐기 §9-1 R8-c 라인·§9-4 순방향 전이 블록 내 `appendConsumptionItemsFromMR` 호출 2건·§9-10-i Item 자동 갱신 규칙. 유지: §9-16 가드 축 분리 [β방향 결정 1 수단]·§10-14 Layer B 진입 조건 [결정 2]·결정 1β/2 자체. 결정 4 Q 무효화 [PENDING Item 자체가 소멸]. Cycle 2 커밋 120f3ca 유효 [가드 함수는 draft 서비스 wire 에서 사용됨]. Cycle 3-B [draft 서비스 wire: guard swap + `basisCountSource` ESTIMATED/FINAL 자동 분기 + `MR(countSource)` 조건 확장] 별도 커밋 예정), 2026-07-30 R8 Cycle 3-B 완료 `531fc0a` (draft 서비스 IN_PROGRESS 지원 wire 실행 — `consumption-draft.service.ts` 가드 교체 [`assertMealPlanCompletedForConsumption`→`assertMealPlanInProgressForDraftView`]·MealPlanGroup status 필터 `{IN_PROGRESS, COMPLETED}` 확장·MR `countSource` 자동 분기 [COMPLETED→FINAL/IN_PROGRESS→ESTIMATED]·반환 객체에 `basisCountSource` 필드 추가·부자재 산출 count 소스 리네임 `finalCountByKey`→`mealCountByKey` [ESTIMATED 시 `estimatedCount` 사용]·`MaterialRequirementNotGeneratedError` 시그니처에 `countSource` 파라미터 추가. `build-consumption-draft.action.ts` 에러 매핑 `MEAL_PLAN_NOT_COMPLETED_FOR_CONSUMPTION`→`MEAL_PLAN_NOT_READY_FOR_DRAFT_VIEW` 교체. 테스트 3건 신규 [COMPLETED→FINAL, IN_PROGRESS→ESTIMATED, IN_PROGRESS 부자재 estimatedCount 산출] + `setupInProgressGroup` 헬퍼. vitest 632→635 passed/2 skipped [+3 순수 증분, 회귀 0]. type-check 0 errors. +169 insertions/-33 deletions [3 files]. 감사서 §9-4 순방향 전이 개정본·§9-16 가드 축 분리·결정 1β 코드 정합 최종. `confirmConsumption` 은 자체 가드 `assertMealPlanCompletedForConsumption` 유지되어 COMPLETED 만 통과 [무회귀 확인]. UI 사용 관리 목록 공백 원인 [Cycle 1 감사서 서술]은 draft 서비스 완화로 IN_PROGRESS 진입 시 draft 조회 가능해져 부분 해소, UI `basisCountSource` 배지 표시는 Cycle 3-C UI 통합에서 마무리), 2026-07-30 R8 Cycle 3-C 완료 `1df7f25` (draft form UI 통합 — `consumption-draft-form.tsx` 헤더 영역에 basisCountSource 배지 삽입 [FINAL=초록 "확정 산출", ESTIMATED=노랑 "임시 산출"], ESTIMATED 시 IN_PROGRESS 안내 배너 표시, Layer B 편집기는 `basisCountSource === "FINAL"` 조건부 렌더링 [ESTIMATED 시 대체 안내 박스 §10-14], 확정 버튼 `disabled` 조건에 `basisCountSource !== "FINAL"` 추가 + tooltip 안내 [결정 2 정합]. Badge import 추가. 서버 방어 [`assertMealPlanCompletedForConsumption` + `assertScopeAccess` in `confirmConsumption` action] 무변경 유지 → 클라이언트·서버 이중 방어 확립. +43 insertions/-11 deletions [1 file]. vitest 635 passed/2 skipped 유지 [UI 컴포넌트 테스트 부재, 무회귀], type-check 0 errors. Cycle 1 감사서 서술 "UI 사용 관리 목록 공백" 원인 최종 해소: IN_PROGRESS 상태에서도 draft 조회 가능하며 사용자는 임시 산출 확인 후 COMPLETED 전환 대기 가능. 남은 R8 하위 작업: R8-b(confirm 재작성) Cycle 4·R8-e(SK 매핑) Cycle 5·R8-d(수동 헤더) Cycle 6·R8-f(테스트 보강) Cycle 7·R8-g 잔여 UI Cycle 8)
...
| P16 | **가용재고 정본 공식** — `가용재고 = (해당 식단·라인업 예약 재고) + (기존 재고: 예약 미걸림 remainingQty) − (다른 일자 예약 재고) − (다른 라인업/공장 예약 재고)`. 예약 재고는 (InventoryLot ACTIVE) + (useDate=대상일) + (referenceType=MATERIAL_REQUIREMENT & MR.lineupId 일치) + (Location·ProductionLine 일치) 4요소 모두 충족 | 2026-07-21 신설, 2026-07-23 §9-3 정정 (referenceType `MEAL_PLAN_SLOT`→`MATERIAL_REQUIREMENT`, MR 기반 축 확립). 음수는 0 clamp. P11 Pre-flight 판정 축. 구현: `src/features/inventory/services/available-stock.service.ts` (R6 신설) |
...
| P20 | **MealPlan 역방향 전이 가드** — COMPLETED→IN_PROGRESS·IN_PROGRESS→CONFIRMED 역방향 시 연계된 Consumption 이 CONFIRMED 이면 차단 (`ConfirmedConsumptionExistsError`). PENDING 이하만 있으면 PENDING Consumption 삭제 + FINAL/ESTIMATED MR soft-delete + 관련 InventoryReservation release (R14 예약 대상) 후 상태 원복. 재확정 시점에 R5-P 재실행 (~~R5-R1 폐기~~, 예약은 R14 입고 확정 시점 자연 재생성) | 2026-07-23 신설 (사용자 확인). 2026-07-30 §0 원칙 1 정합 개정 (R5-R1 폐기 반영). R12 재편집 유예 정책의 순환 흐름 정합. 구현: `revert-guard.service.ts` (R5-R1-B) |
...
**결정 완료 (2026-07-24 R6-Pre-3, Q6 사용자 확인)**: `ConsumptionItem.lineupId` 는 **NOT NULL 컬럼으로 즉시 도입** (R6-B-1). Layer B (수동 추가) 항목이 MealPlan 미경유 케이스가 있어 `MealPlan` 조인 파생만으로는 라인업 추적 불가. 상세 감사서 §9-12-b, §10-15.
...
referenceType = "MATERIAL_REQUIREMENT" 이며 참조된 MaterialRequirement 의 lineupId 일치 (2026-07-23 §9-3 정정)
참조된 MaterialRequirement 의 locationId·productionLineId 일치
음수는 0 clamp
...
| S4-3-c-R3-b | ✅ | ConsumptionHeader 스키마 도입 (Prisma + 마이그레이션) | 완료 (`4f08e69`, 2026-07-22) — 신규 enum `ConsumptionHeaderSource (AUTO_MEAL_PLAN/MANUAL)` + `ConsumptionHeaderStatus (PENDING/CONFIRMED/CANCELLED)` 도입. `ConsumptionHeader` 모델 (12필드, 3인덱스, 1 unique `[mealPlanGroupId, locationId, productionLineId, source]`) 신설. `ConsumptionItem.headerId` NOT NULL FK (onDelete Cascade) · `supplierItemId` NULLABLE FK 신설 (R8 말미 NOT NULL 승격 예정). `ConsumptionItem.status` 는 R5 재편 시 삭제 예정 (R3-b 에서는 deprecated marking 유지 — 옵션 B 채택; 참조 코드 2곳만 존재하여 원자성 유지). `ConsumptionSourceType` 은 Item 레벨 Layer A/B 분류로 유지 (Header.source 와 별개 축). back-relation 6곳: Company/Location/ProductionLine/User×2/SupplierItem/MealPlanGroup. 기존 5건 테스트 데이터는 `prisma migrate reset` 으로 초기화 후 마이그레이션 적용. 마이그레이션 `20260722065856_s4_3_c_r3_b_add_consumption_header`, `SCHEMA_COVERAGE.md` 갱신 완료 |
| S4-3-c-R3-c | ✅ | Header 참조 로직 편입 (서비스 최소 범위) | 완료 (`4f08e69`, 2026-07-22, R3-b 와 통합 커밋) — `confirmConsumption` 트랜잭션 내 `ConsumptionHeader.upsert` 편입. `MealPlanGroup` 은 (companyId, targetDate) 로 조회, Header.productionLineId 는 `CookingPlan.productionLineId` 정본 사용 (단일 productionLine 전제; multi-productionLine 대응은 R5 groupBy 로 이관). AUTO_MEAL_PLAN 경로 idempotent (unique key 재사용). 확정 시점 status PENDING → CONFIRMED, `confirmedAt`/`confirmedByUserId` 기록. `ConsumptionItem.create` 에 `headerId` 필드 지정. `buildConsumptionDraft` 반환 형태 개편·drift 검증 확장·테스트 갱신은 R5 (Layer A 자동 산출) 이관 |
| S4-3-c-R4  | ⏭️ | 기존 데이터 purge (범위 제한) | N/A — R3-b 진행 중 `prisma migrate reset` 으로 DB 초기화되어 흡수됨 (2026-07-22 검증: `consumption_items`/`consumption_lot_details`/`inventoryTransaction`(CONSUMPTION·DISPOSAL)/`inventoryReservation`(CONSUMED) 4개 지표 모두 0건 확인, `prisma/scripts/verify-r4-consumption-residual.ts`). 프로덕션 배포 시점에는 원안 (마이그레이션 `purge_stale_consumption` + `scripts/recompute-lot-remaining.ts`) 재검토 필요 |
| S4-3-c-R5  | ⚠️ 부분 폐기 | R5-P 유지, ~~R5-R1 폐기~~ (2026-07-30, §0 원칙 1) | **R5-P** (`autoCreatePendingConsumptionHeaders`) 유지 — `meal-plan.service.ts:updateMealPlanGroup` (CONFIRMED→IN_PROGRESS 트랜잭션) 편입 완료. 회귀 테스트 기존 5종 (§9-8). **~~R5-R1~~** (`autoReserveFromMaterialRequirements`) — §0 원칙 1 위반으로 **폐기 완료 (2026-07-30, 커밋 `68243b2`)**. 사유: 예약 재고는 실물 자재 입고 확정 시점에만 발생해야 하며, IN_PROGRESS 전환 시점에는 실물 자재 부재로 예약 성립 불가. 예약 생성은 R14 입고 확정 시점으로 완전 이관. 실행 내역: 서비스 파일 `auto-reserve-from-material-requirements.service.ts` 삭제 + 테스트 파일 `auto-reserve-from-material-requirements.service.test.ts` 삭제 + `meal-plan.service.ts` import/호출부 제거 + `meal-plan.service.test.ts` mock/assertion 정리 (vitest 622 → 617 실측, 5건 감소). 상세: 감사서 §0/§9-1/§9-14. 원본 4-Commit 시퀀스(참조용): f37adee → 7aaf064 → a45c641 → 34bb71e → 통합 |
| S4-3-c-R5-R1-B | ⬜ | MealPlan 역방향 전이 가드 + 연쇄 정리 | `revert-guard.service.ts` 신설 (P20). CONFIRMED Consumption 존재 시 throw, 없으면 PENDING Consumption 삭제 + MR soft-delete + Reservation release (R14 예약 대상, §0 원칙 1). 2026-07-30 §0 원칙 1 정합 확인 — release 대상은 R14가 생성한 예약이며 R5-R1 폐기와 무관하게 필요 |
| S4-3-c-R6  | ✅ | 가용재고 정본 서비스 (R6-A 완료, R6-B 별도 페이즈로 세분화) | `available-stock.service.ts` 신설 (P16 공식, aggregate-level, breakdown 4필드) + 단위 테스트 7종 · R6-Pre §10 신설 (`ad9469c`) · R6-Pre 정정 §10 SK 정합 D-R6-f α (`a53a29c`) · R6-Pre-2 `[referenceType, referenceId]` 복합 인덱스 D-R6-e α (`ec03f73`) · R6-A 서비스+테스트 (`801630a`) · R6-Pre-3 감사서 §9-12/13, §10-9 개정/§10-12~18, §11 신설 (본 커밋) |
| S4-3-c-R6-B-1 | ✅ | ConsumptionItem 스키마 확장 (`lineupId` NOT NULL + FK + 인덱스) + 서비스 계층 최소 어댑테이션 (감사서 §10-13 mergeItems 키 확장 · Layer B 임시 guard) | 감사서 §10-15 근거. 마이그레이션 `20260724094111_s4_3_c_r6_b_1_add_consumption_item_lineup_id` + `SCHEMA_COVERAGE.md` #52 갱신. Q6 즉시 NOT NULL 도입 (§9-12-b 정황증거). Layer B `lineupId` 필드 도입은 R6-B-3 대기 (§10-14) |
| S4-3-c-R6-B-2 | ✅ | Pre-flight `getAvailableStock` 호출 리팩토링 | 감사서 §10-9, §10-13 근거. 자재는 `getAvailableStock` (P16 5축 매칭) · 부자재는 `getSubsidiaryAvailableForConsumption` (§10-12 신설, `available-stock.service.ts` 별도 export) 분기. FIFO 차감용 Lot 배열은 별도 쿼리 유지 (§10-3 자재 헬퍼 스펙 상 lots 미포함). `MergedItem`/`mergeItems` 병합 키 확장은 R6-B-1 에서 선반영됨. `LayerBItem` 타입 확장은 R6-B-3 관할. 전체 vitest 590 passed 회귀 없음. |
| S4-3-c-R6-B-3 | ✅ | Layer B UI 라인업 선택 + 스코프 필터 (R7-a 통합) | 감사서 §10-14, §10-17 근거. 신규 액션 `getScopedLineupsForConsumptionAction(targetDate, locationId)` — MaterialRequirement 기준 `distinct(lineupId, productionLineId)` 반환, `applyScopeFilter(userScope, baseWhere)` 로 R7-a 유틸 통합. 신규 스키마 `src/features/consumption/schemas/confirm-consumption.schema.ts` — Zod 로 `layerBItem.lineupId`/`productionLineId` non-null 강제 (§10-14). `confirmConsumptionAction` 에 Zod parse + Layer B 각 항목 `assertScopeAccess({productionLineId})` 추가. `LayerBItem` 타입에 `lineupId`/`productionLineId` 필드 (UI 초기 null 허용, 저장 전 non-null 강제). UI: `consumption-layer-b-editor.tsx` "라인업/생산라인" 컬럼 + `<Select>` 신설 (미선택 시 border-red), `consumption-draft-form.tsx` 저장 전 클라 검증 (`quantity<=0 \|\| !lineupId \|\| !productionLineId`), `page.tsx` draft 로드와 병렬 조회 (`Promise.all`). 테스트 `src/tests/scoped-lineups.action.test.ts` 신규 5 케이스 (전권/COMPANY_ADMIN 필터 병합/empty/VALIDATION/permission throw). vitest 621 passed / 2 skipped [기존 616 + 신규 5], type-check 0 errors 회귀 없음. R6-B 4-tier 완료 [R6-B-1 schema · R6-B-2 service · R6-B-3 UI · R6-B-4 tests]. 상위 커밋: R7-a `1fe1ff6` |
| S4-3-c-R6-B-4 | ✅ | `confirm-consumption.service.test.ts` 신설 (12 테스트 케이스) | 감사서 §10-16 근거. 4개 describe 블록: Layer B guard (§10-14) 1종 · Pre-validation (P11/P14) 4종 · Drift 감지 (D2=α) + Layer A lineup non-null guard (§9-13-e) 3종 · mergeItems 키 확장 (§10-13) + Helper 분기 (§10-3/§10-12) 4종. `src/tests/mocks/prisma.ts`에 `cookingPlan: createModelMock()` 인프라 확장. vitest 604 passed / 2 skipped, type-check 0 errors 회귀 없음 |
| S4-3-c-R7-a | ✅ | 스코프 유틸 최소 구현 (R6-B-3 선행) | `src/lib/auth/scope.ts` (`getUserScope`, `applyScopeFilter`, `assertScopeAccess`) + `ScopeAccessDeniedError` 클래스. 감사서 §10-17(L493), §11-1 근거. `src/tests/scope.service.test.ts` 12 케이스 (level 판정 3 · applyScopeFilter 3 · assertScopeAccess 5 · 통합 1). 현 스키마 제약상 COMPANY 레벨만 실동작, LOCATION/PRODUCTION_LINE는 시그니처 선반영 (TODO 주석). |
| S4-3-c-R7-b~e | ⬜ | 전 도메인 스코프 필터 적용 | Consumption/PO/Receiving/Inventory 목록·write 액션. R6-B 와 병행 가능 |
| S4-3-c-R8  | ⬜ | Consumption 서비스 재작성 (β방향 사전 박제 완료 `656c4a6`, 2026-07-30) | `buildConsumptionDraft` (품목 축, 8개 표준 항목, 이론량 제거) + `confirmConsumption` (Header 상태 전이, USED/DISPOSED 분리, FIFO SupplierItem 단위) ~~`append-consumption-item.service.ts` (축 A, §9-15 R8-c)~~ — 폐기 (Cycle 3-A hot-fix `27f5eab`, draft 서비스가 MR 기반 실시간 재계산이라 PENDING Item 사전 생성 불필요) + `create-manual-consumption-header.service.ts` (축 B, §10-14 진입 조건: COMPLETED 만 허용) + `subsidiary-supplier-item-mapping.service.ts` (SK 매핑) + 가드 축 분리 (§9-16: `assertMealPlanInProgressForDraftView` NEW [draft 조회용 IN_PROGRESS+ 허용, `MEAL_PLAN_NOT_READY_FOR_DRAFT_VIEW`], `assertMealPlanCompletedForConsumption` 기존 [사용량 확정·Layer B용 COMPLETED 유지]) + 서비스 테스트 재작성. 사전 결정 (2026-07-30): 결정 1 β · 결정 2 · ~~결정 4 Q~~ (Cycle 3-A 에서 무효화, PENDING Item 자체가 소멸). 착수 순서 (Cycle 3-A 정정, Cycle 3-B/3-C 진행 반영): ✅ R8-a(draft 재작성, guard swap + basisCountSource 자동 분기, Cycle 3-B `531fc0a`) → R8-b(confirm 재작성) → R8-e(SK 매핑) → R8-d(수동 헤더) → R8-f(테스트) → 🟡 R8-g(UI, basisCountSource 배지 부분 완료 Cycle 3-C `1df7f25`, confirm/수동/SK UI 는 R8-b/d/e 완료 후 마무리). ~~R8-c(append-item) → FINAL 재계산 wire~~ 폐기. Cycle 1(감사서 개정, 코드 변경 없음) `656c4a6` 완료, Cycle 2(가드 함수 축 분리 코드 신설) 완료 `120f3ca` — `assertMealPlanInProgressForDraftView` 신규 (IN_PROGRESS/COMPLETED 허용) + `MealPlanNotReadyForDraftViewError` 신규 + `MEAL_PLAN_NOT_READY_FOR_DRAFT_VIEW` 에러 코드 상수 추가. 기존 `assertMealPlanCompletedForConsumption` 무변경, wire 는 Cycle 3(R8-a draft 재작성)에서 진행. vitest 632 passed / 2 skipped 회귀 없음, type-check 0 errors, +91 insertions / -0 deletions. Cycle 3-A(감사서 hot-fix, R8-c 폐기) 완료 `27f5eab` — §9-15/§9-1 R8-c/§9-4 호출 2건/§9-10-i 연쇄 폐기, §9-16·§10-14·결정 1β/2 유지, 결정 4 Q 무효화.  Cycle 3-B(draft 서비스 wire, R8-a 실행) 완료 `531fc0a` — 가드 교체·status 필터 확장·MR countSource 자동 분기·`basisCountSource` 반환 필드·부자재 산출 count 소스 리네임·에러 매핑 갱신. vitest 632→635 passed/2 skipped, type-check 0 errors, +169/-33 [3 files]. `confirmConsumption` 무변경(COMPLETED 전용 가드 유지). Cycle 3-C(draft form UI 통합, R8-g 부분 실행) 완료 `1df7f25` — 헤더 basisCountSource 배지(FINAL 초록/ESTIMATED 노랑) + ESTIMATED 안내 배너 + Layer B 편집기 조건부 렌더링(§10-14) + 확정 버튼 disabled 조건 확장. Badge import 추가. 서버 방어 무변경 → 이중 방어 확립. +43/-11 [1 file]. vitest 635 유지 무회귀, type-check 0 errors. UI 공백 원인 최종 해소. Cycle 4(R8-b: confirmConsumption 재작성 — Header 상태 전이, USED/DISPOSED 분리, FIFO SupplierItem 단위) 착수 대기 |
...
| S4-3-c-R14 | ✅ | **입고 확정 시 예약 자동 생성 (예약 생성의 유일 지점)** | 감사서 §0/§9-14/§11-2 근거 (2026-07-30 개정). 신규 서비스 `src/features/inventory/services/auto-reserve-on-receiving-confirm.service.ts` — `autoReserveOnReceivingConfirm(tx, {companyId, receivingNoteId, actorUserId})` 시그니처, 반환 `{reserved, skipped}`. 트리거: `confirmReceivingNote` 트랜잭션 내 `ReceivingNote.status="CONFIRMED"` 업데이트 + `transitionPurchaseOrderStatus` 직후 (`receiving-note.service.ts`). 로직: 각 `ReceivingNoteItem`별 `purchaseOrderItem.materialRequirementId` 조회 → null 이면 skip (수동 발주), 존재 시 MR 경유 `MealPlanGroup.planDate` 파생 → `InventoryLot` 역방향 조회 (`receivingNoteItemId` FK, H-89 실측 정정 커밋 `b000b66`) → 부자재 Lot (`materialMasterId=null`) skip (§10 D-R6-f α) → `inventoryReservation.create` (`referenceType="MATERIAL_REQUIREMENT"`, `referenceId=MR.id`, `inventoryLotId=Lot.id`, `materialMasterId=Lot.materialMasterId`, `quantity=receivedQty` 그대로, `useDate=MR.mealPlanGroup.planDate`). 독립 누적 (§0 원칙 2) — 동일 MR에 대한 재발주·재입고 시 새 Lot마다 별도 예약 생성, 중복 skip/덮어쓰기 금지. 재고 부족 검증 없음 (§0 원칙 3, 입고 확정분 자체가 예약 대상이므로 부족 케이스 원천 불가). 재확정 방어: `ReceivingNoteAlreadyConfirmedError` 로 자연 충족. R5-R1 폐기(2026-07-30)로 예약 생성의 유일 지점. 테스트 `src/tests/auto-reserve-on-receiving-confirm.service.test.ts` 신규 8 케이스 — NoteNotFound throw · 수동 발주 skip · MR soft-delete skip · Lot 미조회 skip · 부자재 Lot skip · 정상 경로 (quantity/useDate 검증) · 동일 MR 다중 항목 독립 누적 · 혼합 케이스 카운트 검증. 기존 `receiving-note.service.test.ts`에 `receivingNote.findFirst` mock (items:[]) 추가 (R14 skip 경로 진입용). vitest 617 → 625 passed / 2 skipped [Phase 2 서비스 도입 시 617 유지, Phase 3 테스트 추가로 +8]. 커밋 체인: 감사서 §9-14-3 스키마 관계 방향 hotfix `b000b66` → 서비스 도입 + `receiving-note.service.ts` wire `417a4bf` → 전용 테스트 스위트 `8768eaa` |
| S4-3-c-R15 | ✅ | MealPlan CANCELLED 전이 시 활성 Reservation 일괄 release | 감사서 §9-5, §11-3 근거. 신규 서비스 `src/features/inventory/services/release-reservations-on-meal-plan-cancel.service.ts` — `releaseReservationsOnMealPlanCancel(tx, {companyId, mealPlanGroupId, actorUserId})` 시그니처. 로직: MealPlanGroup 존재 확인 (companyId 격리 + soft-delete 제외) → 활성 MR id 조회 (`mealPlanGroupId` + `deletedAt: null`) → `inventoryReservation.findMany({referenceType: "MATERIAL_REQUIREMENT", referenceId: {in: mrIds}, releasedAt: null})` 활성 예약 조회 → 각 예약 `releaseReservation({reason: "MANUAL_CANCEL", actorUserId}, tx)` 호출 (`ReservationReleaseReason` 재사용, 신규 enum 값 추가 없음). 감사 로그는 `releaseReservation` 내부 자동 기록 (§S4-0-d). `updateMealPlanGroup` 통합: `isTransitionToCancelled` 조건 신설 (`isStatusChange && existing.status !== CANCELLED && input.status === CANCELLED`), 신규 `prisma.$transaction` 블록에서 `mealPlanGroup.update` + `releaseReservationsOnMealPlanCancel` 순차 실행. MR soft-delete · PENDING Consumption 정리는 범위 밖 (R5-R1-B/R12 전담). Flow 테스트 `src/tests/meal-plan.service.test.ts` R15 describe 3 케이스 (CONFIRMED→CANCELLED 호출 · IN_PROGRESS→CANCELLED 호출 · CANCELLED→CANCELLED 미호출). 서비스 단위 테스트 `src/tests/release-reservations-on-meal-plan-cancel.service.test.ts` 신규 7 케이스 (MealPlanGroupNotFoundForCancelError · MR 0건 → released:0 · MR 있으나 활성 예약 0건 · 활성 예약 N건 → MANUAL_CANCEL·actorUserId 전달 · 예약 조회 필터 [companyId+referenceType+referenceId in mrIds+releasedAt=null] · MR 조회 필터 [companyId+soft-delete 제외] · Group 조회 필터 [companyId+soft-delete 제외]). vitest 625 → 632 passed / 2 skipped [Phase 4 +7]. 커밋 체인: 서비스 도입 `3d1a572` → 서비스 단위 테스트 스위트 `ae71e37`. 2026-07-30 §0 원칙 1 정합 확인 — R5-R1 폐기 이후 release 대상은 R14가 생성한 예약이 유일. 기존 구현 코드 변경 불필요 (`referenceType="MATERIAL_REQUIREMENT"` 축은 R14 예약도 동일하게 사용) |
