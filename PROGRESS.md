...
- **D16-4**: `fromUnit`이 WEIGHT 카테고리가 아니면 차단 (R1-c toUnit='g' 정책과 정합)
  - `excludeValue="g"` 로 toUnit 중복 선택 차단
  - WEIGHT 외 카테고리 선택 시 빨간 경고 + submit 버튼 비활성화
...
- ✅ 수동 UI: 드롭다운 표시, 카테고리별 그룹, 'g' 제외, WEIGHT 외 차단

#### 후속 메모
- `step-mapping-table.tsx` 가 전달하는 `suggestedFromUnit` 이 UnitMaster.code 와 매칭되지 않으면 빈 상태로 열림 → 향후 R1-d 또는 UX 개선 시 fromUnitName ↔ UnitMaster.code 매핑 강화 검토
