...
>
> 마지막 갱신: 2026-06-18 (R1-a 4분류 완료, R1-a-fix-2 통과. R1-b 모드 분리 위저드 계획 수립)
...
- **현재 기준 완료 지점**: Sprint 3 Phase 4-B'-5c-Fix-R1-a-2 (위저드 4분류 + 금액 정수 안정화)
- **최근 완료**:
  - M-Fix / M-Fix-R1 / S-Fix (D14·D15) — MaterialMaster·SubsidiaryMaster·SupplierItem 의 `isActive` 생명주기 도입
  - R1-a (commit `5afb0113`) — 위저드 4분류 (mapped / partial / full / unmapped) + Step 2 카드 4분할 + Step 3 통합 섹션
  - R1-a-fix-2 — `stockOffsetAmount` raw 기준 + `Math.round` 정수 안정화 + invariant 부등식 교정
- **다음 진행 항목**: **R1-b1** (멱등성 키 + 사전 안내) → R1-b2 (모드 선택 UI) → R1-b3 (DELTA) → R1-b4 (REPLACE) → R1-c (Step 3 UX·단위환산 인라인) → Phase 1.6 (D9) → Phase 4-C
- **현재 블로커**: 없음
- **누적 테스트**: 372 PASS / 2 skipped / 0 fail
- **TypeScript errors**: 0
- **백엔드 위저드 파이프라인**: 완성 (Step 1 액션 → MR 조회 → 환산 → 4분류 → 배치 생성 → PriceHistory 적층)
...
| **R1-b1** | 멱등성 키 (`PurchaseOrderBatch` 모델 + `idempotencyKey` unique) + Step 1·5 기존 PO 사전 안내 카드 | 🔄 **착수 대기** | - |
| **R1-b2** | 위저드 모드 선택 UI (NEW / DELTA / REPLACE 라디오) — DELTA·REPLACE 골격만 disabled | ⬜ | - |
| **R1-b3** | DELTA 모드 본격 구현 + `POAdjustmentLog` 모델 + item 병합 + 이력 적층 | ⬜ | - |
| **R1-b4** | REPLACE 모드 본격 구현 (DRAFT 한정, 일괄 CANCELLED 전이 + 신규 PO 원자적 생성) | ⬜ | - |
