...
>
> 마지막 갱신: 2026-07-21 (Sprint 4 Phase S4-3-c 재편 감사서 편입 — R0 착수. 기존 c-4-1~c-4-5 폐기, R0~R13 순차 진행. 상세 감사 이력: `docs/audits/2026-07-21-consumption-receiving-audit.md`)
...
| Phase | 상태 | 명칭 | 산출물 |
|---|---|---|---|
| S4-3-c-R0  | ⏳ 진행중 | 헌법 보강 + 감사서 편입 | PROGRESS.md 갱신 + `docs/audits/2026-07-21-consumption-receiving-audit.md` |
| S4-3-c-R1  | ⬜ | Shipping 코드 잔재 제거 | 라우트·컴포넌트·import 스캔 후 제거 (스키마는 S4-3-INT 완료) |
| S4-3-c-R2  | ⬜ | 기존 c-4-x 롤백 | `unit-conversion-mini-dialog`, `consumption-new-entry-dialog`, `pending-meal-plan-banner`, `theoreticalQty`, `orderUnit` alias, `expectedQty` 잔재 전량 제거 |
| S4-3-c-R3  | ⬜ | ConsumptionHeader 스키마 도입 | 마이그레이션 `add_consumption_header` (source enum + manualReason + createdByUserId 포함) + `SCHEMA_COVERAGE.md` 갱신 |
