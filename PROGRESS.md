...
- **Phase 7-F2/F3 cleanup** 완료: 미사용 `recipeOptions` state 제거 (commit `f56b312`)
- **Phase 8 Step 8-A** 완료: `meal-plan.service.test.ts` 신규 작성 — MealPlanAccessory CRUD + Phase 7-F1 BOM 가드 + MealCount upsert + applyMealTemplate 매핑 등 18 케이스 추가 (commit `6cef14e`)
...
## Phase 8.5: Location / ProductionLine 마스터 완성 (2026-06-05 완료)

Phase 9 (자재요구량 자동 산출) 선행 작업으로, 재고 보유 위치(공장·창고)와 생산라인 마스터를
완성하여 모든 후속 흐름의 기반을 마련했다.

### Step 8.5-A: 스키마 확장 (커밋 `dfdb7f1c`)
- **enum 추가**: `LocationType { FACTORY, WAREHOUSE, HYBRID }`
- **Location 모델 확장**: `type`, `note`, `isActive`, `sortOrder` 필드 + 인덱스 4개
- **ProductionLine 모델 확장**: `code` (자동채번 PL-xxx), `sortOrder`, `note`, `deletedAt` + 인덱스 5개
- **시드 정비**: Location 2건(LOC-001/002), ProductionLine 4건(PL-001~004)
- **마이그레이션 베이스라인 재생성**: `20260604091418_init_baseline_v6_phase_8_5`
  - 기존 마이그레이션 히스토리 squash (개발 단계, DB 리셋 수용)

### Step 8.5-B: Location 마스터 (커밋 `9d8b10b2`, `4977fdc2`)
- **신규 디렉토리**: `src/features/location/` (schemas, services, actions, components)
- **서비스**:
  - 자동채번 `LOC-001` (companyId 단위, `$queryRaw` 정규식)
  - 의존성 가드 6개: ProductionLine, InventoryLot, InventoryTransaction, StockTake,
    InventoryTransfer(from+to), ShippingOrder
- **액션**: 권한 키 `location` 신설 (READ/CREATE/UPDATE/DELETE), AuditLog 통합
- **UI**: `/locations` 페이지 (list/new/edit 3-mode), 유형 필터, 활성 필터
- **사이드바**: "공장/창고 관리" 항목 추가 (Factory 아이콘, 공급업체 다음)

### Step 8.5-C: ProductionLine 마스터 (커밋 `10f4584c`, `3686206b`)
- **신규 디렉토리**: `src/features/production-line/`
- **서비스**:
  - 자동채번 `PL-001` (companyId 단위)
  - **위치 유형 가드**: WAREHOUSE 위치 선택 차단 (`LOCATION_NOT_FACTORY`),
    비활성 위치 차단(`LOCATION_INACTIVE`), 미존재(`LOCATION_NOT_FOUND`)
  - 의존성 가드 2개: CookingPlan, MealPlanSlot
  - `getFactoryLocationOptions`: 폼 드롭다운용 FACTORY/HYBRID 활성 위치 조회
- **액션**: 권한 키 `productionLine` 신설
- **UI**: `/production-lines` 페이지, 공장 필터 + 상태 필터, 폼은 공장 선택 드롭다운 포함
- **사이드바**: "생산라인 관리" 항목 추가 (Cog 아이콘)

### Step 8.5-D: 테스트 + 문서화 (이번 커밋)
- `src/tests/location.service.test.ts`: 자동채번, CRUD, 의존성 가드 케이스 추가
- `src/tests/production-line.service.test.ts`: 위치 유형 가드, 의존성 가드 케이스 추가
- 누적 테스트: 199 → 217 PASS (예상)

### 검증 결과
- TypeScript: 0 오류
- Vitest: 전체 PASS (기존 + 신규)
- 수동 검증: 등록/수정/삭제, 창고 차단, 의존성 차단 UI 모두 정상 동작 확인 (사용자 보고)
- 레포 트리: `src/features/location/`, `src/features/production-line/` 신규 생성 확인,
  `sidebar.tsx`에 두 메뉴 추가 확인 (Factory, Cog 아이콘)
