...
| S4-3-c-R5  | ✅ | R5-P + R5-R1 (auto Pending Header + auto reserve from MR) | `autoCreatePendingConsumptionHeaders` + `autoReserveFromMaterialRequirements` 신설 → `meal-plan.service.ts:updateMealPlanGroup` (CONFIRMED→IN_PROGRESS 트랜잭션) 편입 완료. `referenceType="MATERIAL_REQUIREMENT"`, `referenceId=MaterialRequirement.id` (§9-3). 예약 부족 시 전체 롤백 (§9-4 α3). IN_PROGRESS→COMPLETED 시점 예약 재호출 안 함 (§9-4 α3). 회귀 테스트 기존 5종 + 신규 3종 (§9-8). 4-Commit 시퀀스: f37adee(§9-10+스켈레톤) → 7aaf064(R5-P) → a45c641(R5-R1) → 34bb71e(§9-4 α3 정정) → [본 커밋](통합). 상세: 감사서 §9 |
...
| S4-3-c-R14 | ⬜ | R5-R2 발주→입고 흐름 예약 생성 | 입고 확정(receiving confirm) 트랜잭션 내 `MaterialRequirement` 매칭 후 `InventoryReservation` 자동 생성. 2026-07-23 R5 범위 축소로 분리 신설. R5-R1 완료 후 착수 |
