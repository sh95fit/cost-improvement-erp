...
| S4-3-c-R5  | ⬜ | R5-P + R5-R1 (auto Pending Header + auto reserve from MR) | `autoCreatePendingConsumptionHeaders` + `autoReserveFromMaterialRequirements` 신설 → `meal-plan.service.ts:705-740` (CONFIRMED→IN_PROGRESS 트랜잭션) 편입. `referenceType="MATERIAL_REQUIREMENT"`, `referenceId=MaterialRequirement.id` (§9-3). 예약 부족 시 전체 롤백 (§9-4). 회귀 테스트 3종 (§9-8). 상세: 감사서 §9 |
