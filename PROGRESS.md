...
| P13 | **사용 처리(Consumption) 이중 레이어** — Layer A(식단 자동 산출, 의무) + Layer B(수동 추가, 선택). 사용 처리는 `MealPlan.status=COMPLETED` 이후에만 진입 가능 | Layer A/B 모두 FIFO 소진 + Pre-flight(P11) + 잔여분 재고/폐기 분기. Layer B는 SK 재고·대체 사용·식단 변경 추가분을 모두 포함 (P13 보강 참조) |
...
**S4-0-a — 이원 발주 · 사용 처리 스키마 도입**
- `MaterialMaster.isStockKeeping Boolean @default(false)` + `@@index([isStockKeeping])`
- `PurchaseOrder.purchaseKind PurchaseKind @default(WIZARD)` + `@@index([purchaseKind])`
- `enum PurchaseKind { WIZARD, MANUAL_JIT, STOCK_KEEPING }`
- `ConsumptionItem.sourceType ConsumptionSourceType @default(MEAL_PLAN_AUTO)` + `@@index([sourceType])`
- `enum ConsumptionSourceType { MEAL_PLAN_AUTO, MANUAL_ADDITION }`
- 마이그레이션 백필: 기존 데이터는 `isManual=true` → `purchaseKind=MANUAL_JIT`, 그 외 → `WIZARD`. 기존 `ConsumptionItem` 은 모두 `MEAL_PLAN_AUTO`.
- `SCHEMA_COVERAGE.md` 갱신.

**S4-0-b — InventoryTransaction 부자재 대응**
- `itemType ItemType @default(MATERIAL)` 신규
- `materialMasterId` nullable 전환
- `subsidiaryMasterId String?` 신규 + FK + 인덱스
- `SubsidiaryMaster.inventoryTransactions InventoryTransaction[]` 역방향 관계
- CHECK constraint XOR (마이그레이션 SQL 직접 추가):
...
- 기존 데이터 백필: 전량 `item_type = 'MATERIAL'`.

**S4-0-c — InventoryReservation 활성화**
- 스키마 실측 후 필요 시 필드 보강 (예약 상태·해제 사유 이력 등)
- `getAvailableQty(lotId, tx?)` 헬퍼 신설: `remainingQty − Σ active Reservation.qty`
- 만료 배치 스켈레톤: `expireStaleReservations` (Vercel Cron 후크는 S5-5 에서 연결)
- **주의**: SK 재고는 Reservation 미적용. `PurchaseKind.STOCK_KEEPING` 로트는 예약 생성 대상 제외.

**S4-0-d — AuditLog 헬퍼 표준화**
- `writeAuditLog(tx, { entityType, entityId, action, actorUserId, before?, after?, note? })` 표준 헬퍼
- 기존 산발 호출 지점(발주 상태 전이, 입고 확정, 실사 확정 등)을 이 헬퍼로 통합 리팩터링

---

#### Phase S4-1 · 발주 UI 3분할 완성

**S4-1-a — JIT 수동 발주 폼 부자재 확장**
- `manual-purchase-order-form.tsx` 의 하드코딩 `itemType: "MATERIAL"` 제거
- 행 단위 "품목 유형" 토글 (자재/부자재)
- 자재 선택 시 `getMaterialsAction`, 부자재 선택 시 신규 `getSubsidiariesAction`
- `getSupplierItemsBySubsidiaryAction` 신설 (기존 `getSupplierItemsByMaterialAction` 대칭)
- 저장 payload 에 `purchaseKind: "MANUAL_JIT"` 명시

**S4-1-b — 재고 발주(SK) 폼 신설**
- 신규 라우트 `/purchase-orders/stock/new`
- 폼 구성: 입고지 Location(공장/창고 선택 가능), 발주일, 예상 입고일(선택), 비고
- **라인업·출고예정일·생산라인 필드 없음**
- 품목 행: 자재/부자재 토글. 자재 선택 시 기본 필터는 `isStockKeeping = true` + 부자재 전체. "일반 자재도 표시" 스위치로 필터 해제 가능
- 저장 payload: `purchaseKind: "STOCK_KEEPING"`, `lineupId: undefined`, `outboundDate: undefined`, `mealPlanGroupId: undefined`
- 저장 후 목록으로 리다이렉트 (수동 발주와 동일한 idempotency 정책)

**S4-1-c — Location 관리 UI 최소 확장**
- 현재 `src/features/location/` 하위는 빈 폴더. 최소 CRUD UI 신설:
  - 목록·등록·수정 폼 3종
  - 등록 폼에 `type` 라디오 (FACTORY / WAREHOUSE / HYBRID)
- `getLocationOptionsAction({ types?: LocationType[] })` 신설: SK 발주 폼의 입고지 셀렉트에서 `types: ["FACTORY", "WAREHOUSE", "HYBRID"]` 조회
- 기존 `getFactoryLocationOptionsAction` 는 하위 호환 유지 (내부적으로 `getLocationOptionsAction({ types: ["FACTORY", "HYBRID"] })` 로 위임)

**S4-1-d — 권한 리소스 키 세분화**
- `PermissionSetItem.resource` 컨벤션 확장:
  - `purchaseOrder:wizard`
  - `purchaseOrder:manualJit`
  - `purchaseOrder:stockKeeping`
  - `receivingNote`
  - `receivingNote:warehouse`
  - `consumption`
- Action 레이어 진입 가드에서 리소스 키 분기
- 시드 스크립트: COMPANY_ADMIN 역할에 `receivingNote:warehouse` 기본 부여
- 관련 문서 갱신 (`docs/progress/PERMISSIONS.md` 또는 신설)

**S4-1-e — 발주 서비스 · 배치 스키마 확장**
- `createPurchaseOrdersBatchSchema` 에 `purchaseKind: z.nativeEnum(PurchaseKind).default("WIZARD")` 추가
- `isManual` 은 `purchaseKind` 로부터 파생 (하위 호환)
- 검증 로직:
  - `purchaseKind === "MANUAL_JIT"` → `lineupId` NOT NULL, `outboundDate` NOT NULL
  - `purchaseKind === "STOCK_KEEPING"` → `lineupId`/`outboundDate`/`mealPlanGroupId` 강제 NULL
  - `purchaseKind === "WIZARD"` → `mealPlanGroupId` NOT NULL
- 신규 에러 코드: `PURCHASE_KIND_LINEUP_MISMATCH`, `PURCHASE_KIND_OUTBOUND_MISMATCH`, `STOCK_KEEPING_REQUIRES_LOCATION`, `MANUAL_JIT_REQUIRES_LINEUP`
- 그룹키 규칙 분기 (P12 보강 참조)

**S4-1-f — 발주 목록 UI 필터 확장**
- 기존 `isManual` 필터를 `purchaseKind` 필터로 확장 (드롭다운: 전체 / 위저드 / JIT 수동 / SK)
- 목록 열에 발주 유형 배지 추가
...
**S4-2-b — 관련 테스트**
- 부자재 PO → 입고 확정 → InventoryLot/Transaction 대칭 생성 시나리오
- CHECK constraint 위반 케이스 (수동 SQL 삽입 시 XOR 위반) — 통합 테스트로 확인
- 창고 입고 권한 없는 사용자의 접근 차단 테스트
...
**S4-3-a — 진입 가드 서비스**
- `assertMealPlanCompletedForConsumption(companyId, targetDate, locationId)` 헬퍼
- `MEAL_PLAN_NOT_COMPLETED_FOR_CONSUMPTION` 에러 코드

**S4-3-b — buildConsumptionDraft 서비스**
- 입력: `(companyId, targetDate, locationId)`
- Layer A 자동 산출:
  - 해당 컨텍스트의 `MealPlan × finalCount × BOM` 전개
  - 부자재 중 `MealTemplateAccessory.consumptionType = PER_MEAL_COUNT` 항목 포함
  - `sourceType = MEAL_PLAN_AUTO` 로 초안 반환
- 각 자재/부자재에 레퍼런스 데이터 첨부:
  - 예상 식수, 확정 식수 (해당 MealPlan 헤더)
  - 총 재고량 (`availableQty` — Reservation 제외)
  - 해당 출고일 위해 입고된 수량 (`outboundDate = targetDate` PO 로부터의 입고 합계)
- 반환 형태: `{ header, layerAItems, references }`

**S4-3-c — 사용 처리 화면 신설**
- 라우트: `/consumption/new?date=YYYY-MM-DD&locationId=...`
- 진입 시 `buildConsumptionDraft` 호출, 결과로 자동 채움
- 화면 구성:
  - **헤더**: 출고일, 공장, 예상 식수, 확정 식수, 진행 상태
  - **Layer A 테이블** (자동 산출, 회색 배경):
    - 열: 품목명 / 예상 사용량 / 실사용량(편집) / 폐기량(편집) / 폐기사유(편집) / 총재고 / 당일입고 / 잔여 재고
    - "실사용량 + 폐기량 ≤ 총재고" 자동 검증 표시
  - **Layer B 테이블** (수동 추가, 흰 배경) + "품목 추가" 버튼:
    - 자재/부자재 통합 검색 셀렉트
    - 열 구성 Layer A 와 동일
- 하단: 잔여 재고 요약, 폐기 총액, 저장 버튼
