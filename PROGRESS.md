...
| S4-3-c-R5  | ⚠️ 부분 폐기 | R5-P 유지, ~~R5-R1 폐기~~ (2026-07-30, §0 원칙 1) | **R5-P** (`autoCreatePendingConsumptionHeaders`) 유지 — `meal-plan.service.ts:updateMealPlanGroup` (CONFIRMED→IN_PROGRESS 트랜잭션) 편입 완료. 회귀 테스트 기존 5종 (§9-8). **~~R5-R1~~** (`autoReserveFromMaterialRequirements`) — §0 원칙 1 위반으로 **폐기 (2026-07-30, 감사서 커밋 `68e6726`)**. 사유: 예약 재고는 실물 자재 입고 확정 시점에만 발생해야 하며, IN_PROGRESS 전환 시점에는 실물 자재 부재로 예약 성립 불가. 예약 생성은 R14 입고 확정 시점으로 완전 이관. Phase 1 커밋 예정: 서비스 파일 삭제 + `updateMealPlanGroup` 호출부 제거 + 관련 테스트 3종 삭제 (vitest 622 → 619 예상). 상세: 감사서 §0/§9-1/§9-14. 원본 4-Commit 시퀀스(참조용): f37adee → 7aaf064 → a45c641 → 34bb71e → 통합 |
...
| S4-3-c-R14 | ⬜ | **입고 확정 시 예약 자동 생성 (예약 생성의 유일 지점)** | 감사서 §0/§9-14/§11-2 근거 (2026-07-30 개정). 신규 서비스 `src/features/inventory/services/auto-reserve-on-receiving-confirm.service.ts` — `autoReserveOnReceivingConfirm(tx, {companyId, receivingNoteId, actorUserId})` 시그니처. 트리거: `confirmReceivingNote` 트랜잭션 내 `ReceivingNote.status="CONFIRMED"` 업데이트 직후 (`receiving-note.service.ts` L319 근처). 로직: 각 `ReceivingNoteItem`별 `purchaseOrderItem.materialRequirementId` 조회 → null 이면 skip (수동 발주), 존재 시 MR 경유 `MealPlanGroup.planDate` 파생 → `inventoryReservation.create` (`referenceType="MATERIAL_REQUIREMENT"`, `referenceId=MR.id`, `inventoryLotId=ReceivingNoteItem.inventoryLotId`, `quantity=receivedQty` 그대로, 수량 상한 없음). 독립 누적 (§0 원칙 2) — 동일 MR에 대한 재발주·재입고 시 새 Lot마다 별도 예약 생성, 중복 skip/덮어쓰기 금지. 재고 부족 검증 없음 (§0 원칙 3, 입고 확정분 자체가 예약 대상이므로 부족 케이스 원천 불가). 재확정 방어: `ReceivingNoteAlreadyConfirmedError` 로 자연 충족. R5-R1 폐기(2026-07-30)로 예약 생성의 유일 지점. Phase 2 커밋 예정 |
| S4-3-c-R15 | ✅ | MealPlan CANCELLED 전이 시 활성 Reservation 일괄 release | 감사서 §9-5, §11-3 근거. 신규 서비스 `src/features/inventory/services/release-reservations-on-meal-plan-cancel.service.ts` — `releaseReservationsOnMealPlanCancel(tx, {companyId, mealPlanGroupId, actorUserId})` 시그니처. 로직: MealPlanGroup 존재 확인 (companyId 격리 + soft-delete 제외) → 활성 MR id 조회 (`mealPlanGroupId` + `deletedAt: null`) → `inventoryReservation.findMany({referenceType: "MATERIAL_REQUIREMENT", referenceId: {in: mrIds}, releasedAt: null})` 활성 예약 조회 → 각 예약 `releaseReservation({reason: "MANUAL_CANCEL", actorUserId}, tx)` 호출 (`ReservationReleaseReason` 재사용, 신규 enum 값 추가 없음). 감사 로그는 `releaseReservation` 내부 자동 기록 (§S4-0-d). `updateMealPlanGroup` 통합: `isTransitionToCancelled` 조건 신설 (`isStatusChange && existing.status !== CANCELLED && input.status === CANCELLED`), 신규 `prisma.$transaction` 블록에서 `mealPlanGroup.update` + `releaseReservationsOnMealPlanCancel` 순차 실행. MR soft-delete · PENDING Consumption 정리는 범위 밖 (R5-R1-B/R12 전담). 테스트 `src/tests/meal-plan.service.test.ts` R15 describe 신규 3 케이스 (CONFIRMED→CANCELLED 호출 · IN_PROGRESS→CANCELLED 호출 · CANCELLED→CANCELLED 미호출). vitest 624 passed / 2 skipped [기존 621 + 신규 3], type-check 0 errors 회귀 없음. 상위 커밋: R6-B-3 `d8f3764` | 2026-07-30 §0 원칙 1 정합 확인 — R14 폐기(2026-07-30) 이후 release 대상은 R14가 생성한 예약이 유일. 기존 구현 코드 변경 불필요 (`referenceType="MATERIAL_REQUIREMENT"` 축은 R14 예약도 동일하게 사용).
