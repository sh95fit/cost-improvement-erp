...
- `(locationId, productionLineId)` 조합 추출 경로: `MealPlanGroup → MealPlan[] → MealPlanSlot[]` 순회 후 unique set. Slot 의 `productionLineId` 는 NOT NULL (스키마 확인).
...
**미해결 → 후속**:
- MealPlanGroup 이 여러 planDate 를 가지는 경우 없음 (스키마 `planDate` 단일 필드). MealPlan 별 다중 baseDate 는 현재 스키마상 없음 확인.
