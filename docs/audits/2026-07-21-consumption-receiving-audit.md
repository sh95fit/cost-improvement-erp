...
## §10. R6 상세 설계 (가용재고 정본 서비스, 2026-07-23 사전 결정 박제)
...
### §10-2. P16 공식 축 매칭 정책
- **매칭 축 4개**: `(materialMasterId, locationId, productionLineId, lineupId, useDate)`.
- **lineupId nullable 처리**: `MR.lineupId=null` = STOCK_KEEPING MR (특이사항 2·6). 
  - 같은 축 예약 판정 시 `input.lineupId=null` ↔ `MR.lineupId=null` 은 **매칭 인정** (α 정책).
  - `input.lineupId="X"` ↔ `MR.lineupId=null` 은 **매칭 안 함** (다른 축).
- **productionLineId NOT NULL 확정** (특이사항 1): MR 스키마 상 nullable 아님. null 처리 불필요.
...
- **input**: `{companyId, materialMasterId, locationId, productionLineId, lineupId (null 허용), useDate}`
- **result**: `{available, breakdown: {reservedSameAxis, freeStock, stockKeepingFree, reservedOtherDate, reservedOtherAxis}}`
- **음수 clamp**: `available = max(0, reservedSameAxis + freeStock - reservedOtherDate - reservedOtherAxis)`.
- **STOCK_KEEPING freeStock 포함 여부**: R6-A 는 `stockKeepingFree` 를 breakdown 에 별도 표시하되 **available 합산 정책은 §10-2 축 매칭 결과에 따라 결정** (같은 축이면 포함, 다른 축이면 제외).

### §10-4. 반환 타입 상세 (breakdown 관측성)
- 4요소 분리로 대시보드·Pre-flight 통합·디버깅 지원.
- `stockKeepingFree` 는 특이사항 5 반영 별도 필드 (α 정책, 라인업 무귀속 재고 명시적 표기).
...
- **`isReservationEligibleLot(lot)` (reservation-eligibility.ts)**: 예약 대상 판정. R6 는 freeStock 계산 시 재사용 (STOCK_KEEPING 은 stockKeepingFree 로 분리).
...
- 성능: `InventoryReservation.referenceType` + `referenceId` 인덱스 확인 필요 (없다면 후속 페이즈).
...
- (e) STOCK_KEEPING lot 존재 → stockKeepingFree 별도 카운트.
- (f) 음수 clamp (예약 초과) → available = 0.
- (g) MR 미존재 시 (수동 조회) → freeStock 만.
- (h) lineupId=null (STOCK_KEEPING MR 축) → 같은 null 축만 매칭.

### §10-9. P11 Pre-flight 통합 (R6-B, 별도 페이즈)
- `confirm-consumption.service.ts:156` (Pre-flight P4/P11) 리팩토링 대상.
- R6-A 완료 후 별도 페이즈 R6-B 로 이관 (스코프 격리).

### §10-10. 미해결 항목
- **성능 인덱스**: `InventoryReservation (referenceType, referenceId)` 복합 인덱스 필요 여부 → R6-A 착수 후 EXPLAIN 확인.
- **STOCK_KEEPING MR 정의 재확인**: `MR.lineupId=null` 이 STOCK_KEEPING 인지 다른 케이스도 있는지 → R6-A 착수 전 30-H 로 검증.
...
2. **MR.lineupId NULLABLE**. STOCK_KEEPING MR 로 해석. 같은 null끼리만 매칭 인정.
3. **`sumActiveReservationsForLot` 재사용 불가**. 자재/식단 축 무관 전체 합산이므로 R6 는 별도 aggregate 쿼리 작성.
4. **P11 Pre-flight 이미 구현** (`confirm-consumption.service.ts:156`). R6-B 별도 페이즈에서 R6 서비스 호출로 리팩토링.
5. **STOCK_KEEPING 예약 제외** (`isReservationEligibleLot`, `reservation-eligibility.ts:17`). breakdown 별도 카운트로 관측성 확보.
