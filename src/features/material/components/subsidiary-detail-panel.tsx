...
export function SubsidiaryDetailDialog({ subsidiary, open, onOpenChange, onUpdated }: Props) {
  ...
  useEffect(() => { setDefaultItemId(subsidiary.defaultSupplierItemId); }, [subsidiary.defaultSupplierItemId]);

  const handleSetDefault = async (supplierItemId: string) => {
    setSettingDefault(true);
    try {
      const result = await setSubsidiaryDefaultSupplierItemAction(subsidiary.id, supplierItemId);
  ...
  const handleClearDefault = async () => {
    setSettingDefault(true);
    try {
      const result = await setSubsidiaryDefaultSupplierItemAction(subsidiary.id, null);
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          ...
          <div><p className="text-gray-500">등록일</p><p>{new Date(subsidiary.createdAt).toLocaleDateString("ko-KR")}</p></div>
          <div>
            <p className="text-gray-500">기본 공급업체</p>
            {subsidiary.defaultSupplierItem ? (
              <p className="font-medium">{subsidiary.defaultSupplierItem.supplier.name}<span className="ml-2 text-sm text-gray-500">{subsidiary.defaultSupplierItem.productName} · {formatCurrency(subsidiary.defaultSupplierItem.currentPrice)}</span></p>
  ...
  const renderSuppliersTab = () => {
    ...
    if (supplierItems.length === 0) return <p className="py-8 text-center text-sm text-gray-500">이 부자재에 연결된 공급 품목이 없습니다. 공급업체 관리에서 품목을 등록해 주세요.</p>;
    return (
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{subsidiary.name} <span className="ml-2 text-sm font-normal text-gray-500">{subsidiary.code}</span></DialogTitle>
        </DialogHeader>
