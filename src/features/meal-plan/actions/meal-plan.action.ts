...
import { formatSlotQuantityError } from "../utils/slot-quantity-error-formatter";
...
export async function updateMealPlanGroupAction(
  ...
  try {
    ...
    // ★ Phase 9-C-Fix-R1-4: 슬롯 수량 검증 실패 (K2 + R1-3 통합)
    const formatted = await formatSlotQuantityError(
      msg,
      {
        partial: "GROUP_SLOT_QTY_PARTIAL_INPUT",
        sumMismatch: "GROUP_SLOT_QTY_SUM_MISMATCH",
        multiLine: "GROUP_SLOT_QTY_MULTI_LINE",
      },
      "상태를 변경할 수 없습니다. ",
    );
    if (formatted) {
